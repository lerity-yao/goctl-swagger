			...
			*schema.Properties = append(*schema.Properties, kv)
			...
			for _, tag := range memberTags {
				...
				if len(tag.Options) == 0 {
					...
					if !contains(schema.Required, tag.Name) && !strings.HasPrefix(tag.Name, omitemptyOptionX) && tag.Name != omitemptyOption {
						schema.Required = append(schema.Required, tagNameX)
						continue
					}
				...
				for _, option := range tag.Options {
					if !strings.HasPrefix(option, omitemptyOptionX) {
						required = true
					}
				}
				for _, option := range tag.Options {
					if option == omitemptyOption {
						required = false
					}
				}

				if tag.Name == omitemptyOption {
					required = false
				}

				if strings.HasPrefix(tag.Name, omitemptyOptionX) {
					required = false
				}
...
func parseHeader(m spec.Member, parameters []swaggerParameterObject) []swaggerParameterObject {
	...
	for _, tag := range m.Tags() {
		...
		if len(tag.Options) == 0 {
			...
			if !strings.HasPrefix(tag.Name, omitemptyOptionX) && tag.Name != omitemptyOption {
				sp.Required = true
				continue
			}
		}
